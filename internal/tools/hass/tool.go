...
type Tool struct {
	client     *Client
	wsClient   *hasspkg.WSClient // for sync registry queries
	manager    *hasspkg.Manager  // for subscriptions
...
func (t *Tool) getState(ctx context.Context, in hassInput) (json.RawMessage, error) {
	if in.Entity == "" {
...
func (t *Tool) getStates(ctx context.Context, in hassInput) (json.RawMessage, error) {
...
func (t *Tool) matchStateFilter(state map[string]any, filter string) bool {
	// Check entity_id
	if entityID, ok := state["entity_id"].(string); ok {
		if hasspkg.MatchGlobInsensitive(filter, entityID) {
			return true
		}
	}
	...
	if attrs, ok := state["attributes"].(map[string]any); ok {
		if friendlyName, ok := attrs["friendly_name"].(string); ok {
			if hasspkg.MatchGlobInsensitive(filter, friendlyName) {
				return true
			}
		}
		if deviceClass, ok := attrs["device_class"].(string); ok {
			if hasspkg.MatchGlobInsensitive(filter, deviceClass) {
				return true
			}
		}
	}
...
func (t *Tool) getStateDeviceClass(state map[string]any) string {
	if attrs, ok := state["attributes"].(map[string]any); ok {
		if deviceClass, ok := attrs["device_class"].(string); ok {
			return deviceClass
		}
...
func (t *Tool) callService(ctx context.Context, in hassInput) (json.RawMessage, error) {
	if in.Service == "" {
	...
	if err != nil {
		...
		if hassErr, ok := err.(*Error); ok && hassErr.StatusCode == 400 {
...
func (t *Tool) getServices(ctx context.Context, in hassInput) (json.RawMessage, error) {
...
func (t *Tool) getHistory(ctx context.Context, in hassInput) (json.RawMessage, error) {
	if in.Entity == "" {
...
func (t *Tool) errorResult(errType, message string) (string, error) {
...
func (t *Tool) listDevices(ctx context.Context, in hassInput) (json.RawMessage, error) {
	if t.wsClient == nil {
...
func (t *Tool) listAreas(ctx context.Context, in hassInput) (json.RawMessage, error) {
	if t.wsClient == nil {
		return nil, fmt.Errorf("WebSocket client not configured")
	}
...
func (t *Tool) listEntities(ctx context.Context, in hassInput) (json.RawMessage, error) {
	if t.wsClient == nil {
		return nil, fmt.Errorf("WebSocket client not configured")
	}
...
func (t *Tool) filterDevices(data json.RawMessage, in hassInput) (json.RawMessage, error) {
...
func (t *Tool) filterAreas(data json.RawMessage, in hassInput) (json.RawMessage, error) {
...
func (t *Tool) filterEntities(data json.RawMessage, in hassInput) (json.RawMessage, error) {
...
func (t *Tool) matchAny(in hassInput, values ...any) bool {
	for _, v := range values {
		s, ok := v.(string)
		if !ok || s == "" {
			continue
		}
		if in.Pattern != "" && hasspkg.MatchGlobInsensitive(in.Pattern, s) {
			return true
		}
		if in.Regex != "" {
			// Regex is case-sensitive by default; user can use (?i) if needed
			if matched, _ := hasspkg.MatchRegex(in.Regex, s); matched {
				return true
			}
		}
	}
...
func (t *Tool) subscribe(ctx context.Context, in hassInput) (string, error) {
	if t.manager == nil {
	...
	if in.Regex != "" {
		if _, err := hasspkg.MatchRegex(in.Regex, "test"); err != nil {
			return t.errorResult("error", fmt.Sprintf("invalid regex: %v", err))
		}
	}

	// Create subscription
	sub := hasspkg.NewSubscription(uuid.New().String())
...
func (t *Tool) unsubscribe(ctx context.Context, in hassInput) (string, error) {
	if t.manager == nil {
		return t.errorResult("error", "subscription manager not configured")
	}
...
func (t *Tool) enableSubscription(ctx context.Context, in hassInput) (string, error) {
	if t.manager == nil {
		return t.errorResult("error", "subscription manager not configured")
	}
...
func (t *Tool) disableSubscription(ctx context.Context, in hassInput) (string, error) {
	if t.manager == nil {
		return t.errorResult("error", "subscription manager not configured")
	}
...
func (t *Tool) listSubscriptions(ctx context.Context) (string, error) {
	if t.manager == nil {
		return t.errorResult("error", "subscription manager not configured")
	}
