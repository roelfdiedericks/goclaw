...
// UpdateConfig holds configuration for the update tool.
type UpdateConfig struct {
	Enabled bool `json:"enabled"`
}

const configPath = "tools.goclaw_update"

// ConfigFormDef returns the form definition for this tool's config.
func ConfigFormDef() forms.FormDef {
	return forms.FormDef{
		Title: "GoClaw Update Tool",
...
func handleApply(cmd bus.Command) bus.CommandResult {
	cfg, ok := cmd.Payload.(*UpdateConfig)
	if !ok {
		return bus.CommandResult{
			Success: false,
			Error:   fmt.Errorf("expected *UpdateConfig, got %T", cmd.Payload),
		}
	}

	L_info("goclaw_update: config applied", "enabled", cfg.Enabled)
