...
// EditConfig holds configuration for the edit tool.
type EditConfig struct {
	Enabled bool `json:"enabled"`
}

const configPath = "tools.edit"
...
func handleApply(cmd bus.Command) bus.CommandResult {
	cfg, ok := cmd.Payload.(*EditConfig)
	if !ok {
		return bus.CommandResult{
			Success: false,
			Error:   fmt.Errorf("expected *EditConfig, got %T", cmd.Payload),
		}
	}

	L_info("edit: config applied", "enabled", cfg.Enabled)
