...
// TranscriptConfig holds configuration for the transcript tool.
type TranscriptConfig struct {
	Enabled bool `json:"enabled"`
}

const configPath = "tools.transcript"
...
func handleApply(cmd bus.Command) bus.CommandResult {
	...
	if !ok {
		return bus.CommandResult{
			Success: false,
			Error:   fmt.Errorf("expected *TranscriptConfig, got %T", cmd.Payload),
		}
	}

	L_info("transcript: config applied", "enabled", cfg.Enabled)
