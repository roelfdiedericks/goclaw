...
func (c *HTTPChannel) DeliverGhostwrite(ctx context.Context, u *user.User, message string) error {
	...
	sessions := c.getSessionsForUser(u)
	if len(sessions) == 0 {
		logging.L_debug("http: ghostwrite: no active sessions for user", "user", u.ID)
		return nil
	}
	...
	logging.L_info("http: ghostwrite delivered", "user", u.ID, "sessions", len(sessions), "messageLen", len(message))
	return nil
}
...
func (c *HTTPChannel) getOrCreateSession(sessionID string, u *user.User) *SSESession {
	...
	logging.L_debug("http: session created", "session", sessionID, "user", u.ID, "showThinking", u.Thinking, "thinkingLevel", u.ThinkingLevel)
	return sess
}
...
func (c *HTTPChannel) RegisterConnection(sessionID string, u *user.User, lastEventID int) (*SSESession, *SSEConnection, []BufferedEvent) {
	...
	if lastEventID > 0 {
		...
		logging.L_debug("http: SSE replay events", "session", sessionID, "lastEventID", lastEventID, "replayCount", len(replay))
	}

	logging.L_debug("http: SSE connection registered", "session", sessionID, "user", u.ID)
...
func (c *HTTPChannel) UnregisterConnection(sessionID string, conn *SSEConnection) {
	...
	if sess.activeConn == conn {
		sess.activeConn = nil
		logging.L_debug("http: SSE connection unregistered", "session", sessionID)
	}
}
...
func (s *SSESession) SendEvent(event SSEEvent) {
	...
	if conn != nil {
		...
		default:
			logging.L_warn("http: event dropped (buffer full)", "session", s.SessionID, "eventID", eventID)
		}
	}
}
...
func (c *HTTPChannel) RunAgentRequest(ctx context.Context, sessionID string, u *user.User, message string, images []session.ImageAttachment) error {
	...
	go func() {
		...
		logging.L_debug("http: event stream ended", "session", sessionID[:8]+"...")
	}()
...
func (a *MessageChannelAdapter) SendText(chatID string, text string) (string, error) {
	...
	if sent == 0 {
		logging.L_debug("http: no owner sessions for text message")
		return "http-0 (no sessions)", nil // Don't fail - best effort delivery
	}

	logging.L_debug("http: sent text message", "sessions", sent)
...
func (a *MessageChannelAdapter) SendMedia(chatID string, filePath string, caption string) (string, error) {
	...
	if sent == 0 {
		logging.L_debug("http: no owner sessions for media", "path", absPath)
		return "http-0 (no sessions)", nil // Don't fail - best effort delivery
	}

	logging.L_debug("http: sent media", "path", absPath, "sessions", sent)
	return fmt.Sprintf("http-%d", sent), nil
}
