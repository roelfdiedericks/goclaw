...
const configPath = "channels.http"
...
func handleTest(cmd bus.Command) bus.CommandResult {
	...
	if !ok {
		return bus.CommandResult{
			Success: false,
			Error:   fmt.Errorf("expected *http.Config, got %T", cmd.Payload),
		}
	}
	...
	if currentAddr != "" && normalizeAddr(currentAddr) == normalizeAddr(listen) {
		logging.L_debug("http: test skipped port check (already bound)", "listen", listen)
		return bus.CommandResult{
			Success: true,
	...
	logging.L_debug("http: test passed", "listen", listen)
	return bus.CommandResult{
		Success: true,
...
func handleApply(cmd bus.Command) bus.CommandResult {
	...
	logging.L_info("http: config applied", "enabled", enabled, "listen", cfg.Listen)
