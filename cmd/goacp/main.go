...
func runSchemaScenario(ctx context.Context, conn *acp.ClientSideConnection, sessionID acp.SessionID, scenario string) error {
	...
	case "schema-create-plan":
		prompt = "You are helping me probe Cursor ACP schemas. Before doing any work, create a structured plan for improving docs sidebar navigation and search. Request explicit structured plan approval using the richest plan UX you support. Do not implement any changes."
	case "schema-ask-question":
		prompt = "You are helping me probe Cursor ACP schemas. Before creating any plan or doing any work, ask exactly one blocking multiple-choice clarification question using the richest structured question UX you support. Do not proceed until that question is resolved."
	case "schema-task":
...
func run() int {
	...
	client := &spikeClient{permissionMode: permissionMode, extOutDir: extOutDir}
